...
Alborada is a **white-label IPTV subscription management platform**. It allows a business to:
...
2. Select Plan → Confirmation Page
        ...
        │         ├─ Stripe sends webhook → StripeWebhookController
        │         └─ Payment confirmed → Subscription created as "pending"
        │
        ├──► Bank Transfer
        │         │
        │         └─ Upload bank slip → Subscription created as "pending"
        │
        └──► SSL Commerce
                  │
                  └─ Redirect to SSLCommerz → Callback → Subscription "pending"

3. Admin Reviews Pending Subscription
        │
        ├──► Approve → IptvProvisioningService runs
        │         ├─ Creates Xtream Codes user account
        │         ├─ Stores xtream_username, xtream_password, line_id on subscription
        │         ├─ Generates & emails invoice (PDF via DomPDF)
        │         └─ Sends welcome email with IPTV credentials
        │
        └──► Reject → Notification email sent to customer
...
|--------|---------|---------------|
| Credit/Debit Card | Stripe | Keys stored in `settings` table |
| Bank Transfer | Manual | Admin reviews slip and approves |
| Online Payment | SSL Commerce | `config/sslcommerz.php` |
...
### Invoices
- Auto-generated on subscription approval.
- Invoice numbers follow format `INV-YYYY-XXXXX`.
- PDF generated via **DomPDF** and emailed to the customer.
...
### WHMCS Sync (Optional)
If WHMCS credentials are configured, `WhmcsService` syncs client and subscription data to a WHMCS instance, enabling billing reconciliation.

---

## 7. Reseller System

Resellers operate a **credit-based** sub-platform within Alborada.
